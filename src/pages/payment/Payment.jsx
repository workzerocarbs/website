...
const Payment = () => {
        ...
        setAddressInput(selectedAddress);
         ...
         geocoder.geocode({ address: selectedAddress }, (results, status) => {
             if (status === window.google.maps.GeocoderStatus.OK) {
                 ...
                 handleInputChange({ target: { name: 'address', value: selectedAddress } }, 'address');
    ...
    useEffect(() => {
        if ("geolocation" in navigator) {
            ...
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                const distance = calculateDistance(storeLat, storeLng, latitude, longitude);
   ...
   const handlePlaceOrder = async () => {
    console.log(orderId)
    try {
        const response = await axios.post('https://api.zerocarbs.in/api/web/order/place-order', {
            order_id: orderId,
            items: formattedCart,
        ...
        localStorage.clear();
            ...
            console.log("Order placed successfully:", response?.data?.data.order_id);
        } else {
    ...
    const handleApplyPromoCode = () => {
        ...
        if ( promoCodes.some(promo => promo.title === enteredCode)) {
          ...
          const appliedPromo = promoCodes.find(promo => promo.title === enteredCode);
    ...
    const discountAmount = appliedCoupon ? (appliedCoupon.discount / 100) * total : 0;
...
<div className='row'>
    {!isSmallScreen && <div className='col-md-6 d-flex flex-column gap-2'>
      ...
      <p className='mb-0'>Logged in as ({phoneNumber})</p>
       ...
       {
        !showOTPInputField &&  <div className='d-flex flex-column gap-2' style={{border: "1px solid lightgrey", padding: "20px 15px"}}>
        <div className='d-flex flex-column'>
       <div className='d-flex gap-2 '>
       <IoChevronBackCircleSharp color='#9cd322' size={40}/>
      ...
      style={{ backgroundColor: "#9cd322", display: 'flex', alignItems: 'center', justifyContent: 'center',  cursor: loading ? 'not-allowed' : 'pointer',}}  
      onClick={handleSendOtp} 
      disabled={loading} // Disable button while loading
    >
      {loading ? (
      ...
      {showPickDeliveryAddress &&   <div className='d-flex flex-column gap-1' style={{border: "1px solid lightgrey", padding: "15px 15px", cursor: "pointer"}}>
                ...
                <div className='d-flex justify-content-center align-items-center' style={{border: "1px dashed #007bff", padding: "55px"}} onClick={()=>{
...
{
  showPaymentMethod &&   <PaymentMethod paymentMethod={paymentMethod} handlePaymentChange={handlePaymentChange}/>
}
   ...
   <OrderSummary
    ...
    />
 <PromoCodeComponent/>
    <PaymentSummary itemTotal={itemTotal} gst={gst} packagingCharges={packagingCharges} discountAmount={discountAmount} total={total} appliedCoupon={appliedCoupon}/>
   </div>
</div>
