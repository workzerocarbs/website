...
const MultiSelectCounter = ({ 
  ...
  setTopingsSelect, 
  topingsSelect,
  groupId // Add groupId prop to identify which group the item belongs to
}) => {
  const [count, setCount] = useState(0);

  // Find current group and item
  const currentGroup = topingsSelect.find(group => group.group_id === groupId);
  const currentItem = currentGroup?.addOnItems.find(item => item.id === id);

  // Calculate total quantity in the current group
  const groupTotalQuantity = currentGroup?.addOnItems.reduce(
